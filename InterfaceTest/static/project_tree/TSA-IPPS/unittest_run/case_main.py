...
filename = "/home/ma/PycharmProjects/AutoTest_python/InterfaceTest/project_tree/TSA-IPPS/data_file/case_data_ysc.xlsx"
sheetid_http = 1
sheetid_https = 2

data_http = TsaParamDict(filename,sheetid_http).deal_param(start=0,end=2)
...
class CaseRun(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(self):
        self.itle = '版权服务2.0确权接口测试报告'
        self.description = ""
        self.url = "http://39.107.66.190:9990/v2/api/confirm/opusConfirm" #确权接口  ipp20timestamp.tsa.cn
    ...
    def test_apply_http(self,data_dict):
        ...
        '''
        self.op_excel = OperationExcel(filename, sheetid_http)
        self.url = "http://39.107.66.190:9990/v2/api/confirm/opusConfirm"
        download_url = "http://39.107.66.190:9990/v2/api/confirm/downloadOpusCertificate"
        data_str = datetime.datetime.now().strftime('%Y%m%d')
        download_file = '{}_main_http'.format(data_str)
    ...
    #@ddt.data(*data_https)
    def etest_apply_https(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
        ...
        data_str = datetime.datetime.now().strftime('%Y%m%d')
        download_file = '{}_main_https'.format(data_str)
        pp = pprint.PrettyPrinter(indent=4)
        req_data_dict = deepcopy(data_dict)
        ...
        start = time.time()
        is_pass, serialNo, database_str, database_str_hd, download_req_url, download_req_data, download_res, download_res_data, cmp_req_res, download_case, tsa_file = self.crr.verify_is_pass(
            **kargs)
        count = 1
        while not is_pass and serialNo:
            if download_case:
...
if __name__ == "__main__":
    ...
    description = "确权接口-主流程测试用例-主要验证所有参数合法参数确权成功及必填参数非法数据确权失败"
