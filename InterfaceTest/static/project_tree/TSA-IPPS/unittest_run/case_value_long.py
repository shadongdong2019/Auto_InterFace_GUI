...
data = CaseError(filename,sheetid).make_data_param_value_long_1_fail(59,start=0,end=0)
...
class CaseRun(unittest.TestCase):
    ...
    @ddt.data(*data)
    def test_apply_param_value_long_1_http(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
        ...
        download_file = '{}_param_value_long_1_http'.format(data_str)
    ...
    @ddt.data(*data)
    def test_apply_param_value_long_1_https(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
        ...
        data_str = datetime.datetime.now().strftime('%Y%m%d')
        download_file = '{}_param_value_long_1_https'.format(data_str)
        pp = pprint.PrettyPrinter(indent=4)
        req_data_dict = deepcopy(data_dict)
...
if __name__ == "__main__":
    ...
    description = "申请接口-参数值最大长度+1验证-返回参数不合法（参数：partnerID/partnerKey/salt校验正确性所以不进行超长校验;参数：file/authProtocol传文件流不限制长度不进行校验）"
