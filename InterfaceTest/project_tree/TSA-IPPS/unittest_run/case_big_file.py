...
class CaseRun(unittest.TestCase):
    ...
    @ddt.data(*data_http)
    def test_apply_big_file_http(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
        ...
        download_file = '{}_big_file_http'.format(data_str)
    ...
    def test_apply_hz_https(self,data_dict):
        ...
        data_str = datetime.datetime.now().strftime('%Y%m%d')
        download_file = '{}_big_file_https'.format(data_str)
        pp = pprint.PrettyPrinter(indent=4)
        req_data_dict = deepcopy(data_dict)
...
if __name__ == "__main__":
    ...
    description = "申请接口-参数为大文件验证-作品文件file保证最大可上传50M文件，授权协议authProtocol保证最大可传上10M文件，如超过限制会返回参数不合法"
