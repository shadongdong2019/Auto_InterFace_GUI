...
data = CaseError(filename,sheetid).make_data_param_value_space_fail(1,start=0,end=0)
...
class CaseRun(unittest.TestCase):
    ...
    @ddt.data(*data)
    def test_apply_param_value_space_http(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
        ...
        download_file = '{}_param_value_space_http'.format(data_str)
        ...
        ff = [None,"","   "]
        if req_data_dict.get("file",None) not in ff:
            del req_data_dict["file"]
            pp.pprint("注：作品文件（file）值过大会影响测试报告打开速度，测试报告仅显示用例中的文件地址")
            req_data_dict["file"] = fileB
        if req_data_dict.get("authProtocol",None) not in ff :
            del req_data_dict["authProtocol"]
            req_data_dict["authProtocol"] = authProtocolB
            pp.pprint("注：授权协议（authProtocol）值过大会影响测试报告打开速度，测试报告仅显示用例中的文件地址")
    ...
    @ddt.data(*data)
    def test_apply_param_value_space_https(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
        ...
        data_str = datetime.datetime.now().strftime('%Y%m%d')
        download_file = '{}_param_value_space_https'.format(data_str)
        pp = pprint.PrettyPrinter(indent=4)
        req_data_dict = deepcopy(data_dict)
...
if __name__ == "__main__":
    ...
    description = "申请接口-参数值为空格验证-必填参数返回参数不完全，非必填参数申请成功"
