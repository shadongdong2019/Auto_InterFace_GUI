...
data = CaseError(filename,sheetid).make_data_param_value_None_fail(1,start=0,end=0)
...
class CaseRun(unittest.TestCase):
    ...
    @ddt.data(*data)
    def test_apply_param_value_None_http(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
        ...
        download_file = '{}_param_value_None_http'.format(data_str)
    ...
    @ddt.data(*data)
    def test_apply_param_value_None_https(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
        ...
        data_str = datetime.datetime.now().strftime('%Y%m%d')
        download_file = '{}_param_value_None_https'.format(data_str)
        pp = pprint.PrettyPrinter(indent=4)
        req_data_dict = deepcopy(data_dict)
...
if __name__ == "__main__":
    ...
    description = "申请接口-参数值为空时校验-必填参数返回参数不完全，非必填参数申请成功"
