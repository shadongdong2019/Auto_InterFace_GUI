...
data = CaseError(filename,sheetid).make_data_param_value_type_fail(1,start=0,end=0)
...
class CaseRun(unittest.TestCase):
    ...
    @ddt.data(*data)
    def test_apply_param_value_type_http(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
        ...
        download_file = '{}_param_value_type_http'.format(data_str)
    ...
    @ddt.data(*data)
    def test_apply_param_value_type_https(self,data_dict):
        '''
        测试数据={0}
        :param data_dict:
        :return:
...
if __name__ == "__main__":
    ...
    description = "申请接口-参数值类型错误-仅验证参数类型为整型的情况"
